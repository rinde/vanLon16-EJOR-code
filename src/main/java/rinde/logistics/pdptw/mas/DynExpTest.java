...
// import rinde.sim.pdptw.scenario.ScenarioGeneratorOld;
// import rinde.sim.pdptw.scenario.ScenarioIO;
// import rinde.sim.pdptw.scenario.times.TimeSeriesGenerator;
// import rinde.sim.pdptw.scenario.times.PoissonProcess;
// import rinde.sim.pdptw.scenario.times.SineIntensity;
// import rinde.sim.pdptw.scenario.tw.UrgencyProportionateUniformTWGenerator;
...
// }
// scenarios
// .add(ScenarioIO.read(sb.toString(), VanLon14Scenario.class));
// }
// final ObjectiveFunction objFunc = new Gendreau06ObjectiveFunction();
// final ExperimentResults results = Experiment.build(objFunc)
// .addScenarios(scenarios).withThreads(22).withRandomSeed(123)
// .repeat(1)
// // .showGui()
//
// // CLOSEST
...
// .perform();
//
// final HashMultimap<MASConfiguration, DataPoint> map = HashMultimap
// .create();
...
// });
//
// final String name = config instanceof TruckConfiguration ?
// ((TruckConfiguration) config).rpSupplier
// .toString()
// : config.toString();
// try {
// Files.write(Joiner.on("\n").join(datapoints), new File(dir
// + name + ".txt"), Charsets.UTF_8);
// } catch (final IOException e) {
// throw new IllegalStateException(e);
// }
//
...
// ScenarioGeneratorOld.Builder.DEFAULT_VEHICLE_SPEED;
...
// final long maxRequiredTime =
// (ScenarioGeneratorOld.Builder.DEFAULT_MIN_RESPONSE_TIME
...
//
// final ScenarioGeneratorOld<VanLon14Scenario> scenGen = ScenarioGeneratorOld
// .builder(
// new VanLon14ScenarioFactory(
// ExperimentClass.MEDIUM_MEDIUM))
// .setArrivalTimesGenerator(
//
// new TestArrivalTimes(orders, -.8 + j * .2,
// j * .10d, latestOrderAnnounceTime))
// .setScale(.1d, 10d)
// .setTimeWindowGenerator(
// new UrgencyProportionateUniformTWGenerator(
// depotLocation, length, serviceTime * 60000,
// minResponseTime, maxResponseTime,
// vehicleSpeed)).setScenarioLength(length)
// .build();
//
// try {
// final File dir = new File("files/dataset/dynexp/urgency/");
// Files.createParentDirs(dir);
// checkState(dir.exists() || dir.mkdir(),
// "Could not create dir %s.", dir);
// for (int i = 0; i < 10; i++) {
// final VanLon14Scenario scen = scenGen.generate(rng);
// final double dyn = Metrics.measureDynamism(scen);
//
// final String scenarioName = "scen" + j + "-" + i + "-"
// + dyn;
//
// // Analysis.writeLoads(Metrics.measureRelativeLoad(scen),
// // new
// // File(dir,
// // scenarioName + ".load"));
// Analysis.writeLocationList(Metrics.getServicePoints(scen),
// new File(dir, scenarioName + ".points"));
// Analysis.writeTimes(scen.getTimeWindow().end, Metrics
// .getArrivalTimes(scen), new File(dir, scenarioName
// + ".times"));
...
// .put("vehicle_speed_kmh",
// Metrics.getVehicleSpeed(scen))
// .put("dynamism", dyn);
//
// final ImmutableMultiset<Enum<?>> eventTypes = Metrics
// .getEventTypeCounts(scen);
// for (final Multiset.Entry<Enum<?>> en : eventTypes
// .entrySet()) {
...
// properties.put("parcel_to_vehicle_ratio",
// parcelToVehicleRatio);
//
// // System.out.println(parcelToVehicleRatio + " " +
// // eventTypes
// // .count(PDPScenarioEvent.ADD_PARCEL) + " "
// // + eventTypes.count(PDPScenarioEvent.ADD_VEHICLE));
//
// Files.write(Joiner.on("\n").withKeyValueSeparator(" = ")
// .join(properties.build()), new File(dir,
// scenarioName + ".properties"), Charsets.UTF_8);
//
// Files.write(ScenarioIO.write(scen), new File(dir,
// scenarioName + ".scenario"), Charsets.UTF_8);
//
// }
// } catch (final IOException e) {
// throw new IllegalStateException(e);
...
// }
//
// static class TestArrivalTimes implements TimeSeriesGenerator {
// final TimeSeriesGenerator generator;
//
// public TestArrivalTimes(int orders, double relHeight, double min,
// long length) {
// // final double freq = 1d / 3600000d;
// // final SineIntensity intensity = new SineIntensity(1d, freq,
// // relHeight,
// // min);
// // final UnivariateIntegrator ri = new RombergIntegrator(16, 32);//
// // new
// //
// // final double val = ri.integrate(10000000, new
// // IntensityFunctionWrapper(
// // intensity), 0, length);
// // final double newAmpl = orders / val;
// //
// // final SineIntensity finalIntensity = new SineIntensity(newAmpl,
// // freq,
// // relHeight, newAmpl * min);
// // generator = new NHPoissonProcess(length, finalIntensity);
//
// generator = PoissonProcess.nonHomogenous(length, SineIntensity.builder()
// .period(3600000d).height(relHeight).area(orders)
// .build());
//
// }
//
